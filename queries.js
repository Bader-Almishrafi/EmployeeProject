...
//     const imagePath = req.file ? `/uploads/${req.file.filename}` : null; // Handle optional image
...
//     const salaryWithDeduction = salary - salary * 0.01;

//     db.query(
//         "INSERT INTO employees (fname, lname, position, salary, image_path) VALUES ($1, $2, $3, $4, $5) RETURNING id",
//         [fname, lname, position, salaryWithDeduction, imagePath],
//         (error, results) => {
//             if (error) {
...
const createEmp = (request, response) => {
    ...
    db.query(
        ...
        (error) => {
            ...
            db.query(
                "INSERT INTO employees (fname, lname, position, salary, image_path) VALUES ($1, $2, $3, $4, $5) RETURNING id",
                [fname, lname, position, salary-(salary*0.01), imagePath],
                (error, results) => {
                    if (error) {
                        console.error("Error adding employee:", error);
                        const responseObj = new Response(false, 500, "Server Error", null);
